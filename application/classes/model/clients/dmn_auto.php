<?php
	$objLeads_submit = ORM::factory('leadsubmit');
      $ping_url = 'https://leads.dmnleads.com/api.php';//DynamicPricePing
      $post_url = 'https://leads.dmnleads.com/api.php';
      ...
      if(current($ResidenceYears_temp)==0)
        ...
        $PingID='';
	  $key = '-7rwW2D9rUDPOTDYQ2Ocr25dreNZQ0zcreSYQ26BMBz9rByZWhMZtqNovzii';
	  ...
	  $ping_data = 'Key='.$key.'&API_Action=pingPostLead&Mode=ping&TYPE=65&SRC=onlineautomedia_af&State='.$state.'&Zip='.$zip.'&SSN='.$ssn.'&Monthly_Income='.$monthly_income.'&Test_Lead=1&Return_Best_Price=1';
      ...
      $ping_res = $this->curlposting($ping_url, $ping_data, $header);
        $myFile_post = "ping_post_request/ping_dmn_auto".$lead_data->lead_auto_id.".txt";
     ...
     { 
		...
		$post_data = 'Key='.$key.'&API_Action=pingPostLead&Mode=post&Lead_ID='.$lead_id[1].'&TYPE=65&IP_Address='.$ip_address.'&Landing_Page=landing&First_Name='.$first_name.'&Last_Name='.$last_name.'&Address='.$address.'&City='.$city.'&Email='.$email.'&Phone='.$home_phone.'&Alternate_Phone='.$home_phone.'&CoSigner=Yes&DOB='.$dob.'&Months_At_Residence='.$ResidenceMonths.'&Years_At_Residence='.$ResidenceYears_value.'&Monthly_Mortgage_Or_Rent='.$ResidenceCost.'&Rent_Or_Own='.ucfirst($ResidenceType).'&Bankruptcy=No&Employer='.$employer_name.'&Occupation='.$job_title.'&State='.$state.'&Zip='.$zip.'&SSN='.$ssn.'&Monthly_Income='.$monthly_income.'&Alternate_Phone='.$home_phone.'&Credit_Check_Authorized=YES&Years_Employed='.$yearsAtEmployer.'&Months_Employed='.$monthsAtEmployer;
        ...
        $post_res = $this->curlposting($post_url, $post_data, $header);  
            ...
            preg_match('/<Error>(.*)<\/Error>/' , $post_res , $error);
                        ...
                        //print_r($status_post);
            $myFile_post = "ping_post_request/post_dmn_auto".$lead_data->lead_auto_id.".txt";
            $fh_post = fopen($myFile_post, 'w') or die("can't open file");
