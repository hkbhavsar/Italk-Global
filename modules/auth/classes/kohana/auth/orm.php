...
class Kohana_Auth_ORM extends Auth {
	...
	protected function _login($user, $password, $remember)
	{
		...
		if (($user->has('roles', ORM::factory('role', array('name' => 'login'))) || $user->has('roles', ORM::factory('role', array('name' => 'admin'))) || $user->has('roles', ORM::factory('role', array('name' => 'qc')))) AND $user->password === $password)
		{
			if ($remember === TRUE)
			{
				$this->remember($user);
	...
	{
        Session::instance()->set('chk_last_login',$user->last_login);
		$user->complete_login();
